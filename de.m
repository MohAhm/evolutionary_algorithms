...
% F = (2) * rand(1) % F: Scaling factor, range [0,2] 
% Pr = (1) * rand(1) % Pr: Recombination Probability, range [0,1] 

F = 0.7; % F: Scaling factor, range [0,2] 
Pr = 0.2; % Pr: Recombination Probability, range [0,1] 
...
for i = 1:numF
     ...
     for t = 1:nTimes
          ...
          % for idx = 1 : dimension
          %      if rand < Pr || idx == randi(dimension)
          %           offspring(idx) = mutatedOffspring(idx);
          %      else
          %           offspring(idx) = individual(idx);
          %      end
          % end
